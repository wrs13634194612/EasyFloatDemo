...
internal class FloatingWindowHelper(val context: Context, var config: FloatConfig) {
    ...
    private var touchUtils: TouchUtils? = null
     ...
     */
    private fun addView() {
        if (params == null || windowManager == null || touchUtils == null) return
    ...
    private fun enterAnim(floatingView: View) {
        if (frameLayout == null || config.isAnim || params == null || windowManager == null) return
     ...
     */
    fun exitAnim() {
        if (frameLayout == null || (config.isAnim && enterAnimator == null) || params == null || windowManager == null){
            Log.d("wenTest", "exitAnim: error")
            return
        }
    ...
    fun updateFloat(x: Int, y: Int) {
        if (touchUtils == null || params == null || windowManager == null) return
        frameLayout?.let {
            if (x == -1 && y == -1) {
                // 未指定具体坐标，执行吸附动画
